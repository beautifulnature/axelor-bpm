...
public class BPMN2_0XMLModelType extends SignavioModelType {
	...
	public File storeModel(String path, String id, String name, String description,
		...
		File file = super.storeModel(path, id, name, description, type, jsonRep, svgRep, axelorPath, sessionId);
		try {
			String bpmn20Path = path;
            BPMN20XMLFileUtil.storeBPMN20XMLFile(axelorPath, jsonRep,sessionId,path);
        } catch (IOException e) {
                throw new IllegalStateException("Cannot save BPMN2.0 XML", e);
        } catch (JSONException e) {
        	throw new IllegalStateException("Cannot save BPMN2.0 XML", e);
