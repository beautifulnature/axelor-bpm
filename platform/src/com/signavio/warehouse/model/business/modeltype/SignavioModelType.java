...
public class SignavioModelType implements ModelType {
	...
	public String getDescriptionFromModelFile(String path) {
		return FileSystemUtil.readXmlNodeChildFromFile(XPATH_PREFIX + DESCRIPTION_ElEMENT_NAME, path, null, null ,null);
	}
	
	public String getTypeStringFromModelFile(String path) {
		return FileSystemUtil.readXmlNodeChildFromFile(XPATH_PREFIX + TYPE_ElEMENT_NAME, path, null, null ,null);
	}
	...
	public byte[] getRepresentationInfoFromModelFile(RepresentationType type, String path) {
		try {
			switch (type) {
			case JSON :
				String json = FileSystemUtil.readXmlNodeChildFromFile(XPATH_PREFIX + JSON_ElEMENT_NAME, path, null, null, null);
				if (json != null) {
					return json.getBytes("utf-8");
				}
				break;
			case SVG :
				String svg = FileSystemUtil.readXmlNodeChildFromFile(XPATH_PREFIX + SVG_ElEMENT_NAME, path, null, null, null);
				if (svg != null) {
	...
	public File storeModel(String path, String id, String name, String description,
			String type, String jsonRep, String svgRep, String axelorPath, String sessionId) {
		File modelFile;
//		BPMN20XMLFileUtil bpmn = new BPMN20XMLFileUtil();
//		try{
//			bpmn.storeBPMN20XMLFile(axelorPath, jsonRep, sessionId,path);
//		}
//		catch(Exception e){
//			e.printStackTrace();
//		}
		if ((modelFile = FileSystemUtil.createFile(path, this.getInitialModelString(id, name, description, type, jsonRep, svgRep),axelorPath,sessionId))
				!= null) {
			return modelFile;
		} else {
