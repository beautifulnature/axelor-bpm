...
public class ModelHandler extends BasisHandler {
	...
	public <T extends FsSecureBusinessObject> Object putRepresentation(T sbo, Object params, FsAccessToken token) {
		return postRepresentation(params, token);
...
//		return getRepresentation(model, params, token);
	...
	public Object postRepresentation(Object params, FsAccessToken token) {
		...
		String id=null, copy=null,parentId, name, description, type, comment, jsonRep, svgRep, axelorPath, sessionId;
		FsDirectory parent;
		try {
			parentId = jsonParams.getString("parent");
			parentId = parentId.replace("/directory/", "");
			axelorPath = jsonParams.getString("axelorPath");
			sessionId = jsonParams.getString("sessionId");
		...
		FsModel model;
		parent = new FsDirectory(id);
		try { 
			if(id == null) {
				model = parent.createModel(name, description, type, jsonRep, svgRep, comment, axelorPath, sessionId);
			} else {
				model = parent.createModel(id, name, description, type, jsonRep, svgRep, comment, axelorPath, sessionId);
				this.getServletContext().setAttribute(id, model.getId());
			}
		} catch (Exception e) {
