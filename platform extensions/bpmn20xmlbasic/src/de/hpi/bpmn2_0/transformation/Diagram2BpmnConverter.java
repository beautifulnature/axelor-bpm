...
public class Diagram2BpmnConverter {
	...
	private void detectConnectors() {
		for (GenericEdge<?,?> edge : DiagramHelper.getAllEdges(this.diagram)) {
			...
			if (target != null) {
				...
				if(!edge.getProperty("condition").equals(""))
					edgeElement.setCondition(edge.getProperty("condition"));
				if(!edge.getProperty("role").equals(""))
					edgeElement.setRole(edge.getProperty("role"));
				if(!edge.getProperty("sequence").equals(""))
					edgeElement.setSequence(edge.getProperty("sequence"));
				if(!edge.getProperty("signal").equals(""))
					edgeElement.setSignal(edge.getProperty("signal"));
	...
	private void identifyProcesses() {
		...
		while (allNodes.size() > 0) {
			...
			currentProcess.setCondition(this.diagram.getCondition());
			currentProcess.setModel(this.diagram.getModel());
			currentProcess.setSequence(this.diagram.getSequence());
			String documentation = this.diagram.getProperty("documentation");
			if (documentation != null && !(documentation.length() == 0) && currentProcess.getDocumentation().size() == 0)
				currentProcess.getDocumentation().add(new Documentation(documentation));
